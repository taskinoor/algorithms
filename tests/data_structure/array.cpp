...
TEST(ExtendableArray, InvalidIndex) {
    ...
    ASSERT_THROW(array.get(-1), alg::InvalidIndexError);
    ASSERT_THROW(array.get(0), alg::InvalidIndexError);

    ASSERT_THROW(array.add(-1, 2), alg::InvalidIndexError);
    ASSERT_THROW(array.add(1, 2), alg::InvalidIndexError);

    ASSERT_THROW(array.set(-1, 2), alg::InvalidIndexError);
    ASSERT_THROW(array.set(0, 2), alg::InvalidIndexError);
