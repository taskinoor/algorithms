...
TEST(Matrix, Dimension) {
    alg::numeric::Matrix<double> A(17, 13);
...
template <class T>
alg::numeric::Matrix<T> matrix_test_helper(alg::numeric::Matrix<T> A) {
    return A;
}

TEST(Matrix, CopyAndMoveConstructor) {
    alg::numeric::Matrix<int> A = {{2, 5}};
    alg::numeric::Matrix<int> B = matrix_test_helper(A);
...
TEST(Matrix, CopyAssignmentOperator) {
    alg::numeric::Matrix<double> A = {{2.3}};
    alg::numeric::Matrix<double> B = {{5.7}};
...
TEST(Matrix, OriginalDataAfterMove) {
    alg::numeric::Matrix<int> A = {{1, 2, 3}, {4, 5, 6}};
    ...
    alg::numeric::Matrix<int> B = std::move(A);
...
TEST(Matrix, Transpose) {
    ...
    alg::numeric::Matrix<int> AT = A.transpose();
...
TEST(Matrix, AdditionSubtraction) {
    ...
    alg::numeric::Matrix<int> M = A + B - C + D;
...
TEST(Matrix, Multiplication) {
    ...
    alg::numeric::Matrix<double> M = A * B * C;
...
TEST(Matrix, InvalidArgumentConstructor) {
    ASSERT_THROW(alg::numeric::Matrix<int> M(0, 0), std::invalid_argument);
    ASSERT_THROW(alg::numeric::Matrix<int> M(0, 1), std::invalid_argument);
    ASSERT_THROW(alg::numeric::Matrix<int> M(1, 0), std::invalid_argument);

    ASSERT_NO_THROW(alg::numeric::Matrix<int> M(1, 1));
}

TEST(Matrix, InvalidArgumentInitializerList) {
    ASSERT_THROW(alg::numeric::Matrix<int> M = {{}}, std::invalid_argument);

    try {
        alg::numeric::Matrix<int> M = {
            {2},
            {3, 5}
        };
...
TEST(Matrix, InvalidArgumentCopyAssignment) {
    alg::numeric::Matrix<int> A(2, 3);
    alg::numeric::Matrix<int> B(5, 3);
    alg::numeric::Matrix<int> C(2, 7);
    alg::numeric::Matrix<int> D(2, 3);
...
TEST(Matrix, InvalidArgumentMultiplication) {
    alg::numeric::Matrix<int> A(2, 3);
    alg::numeric::Matrix<int> B(3, 2);
    alg::numeric::Matrix<int> C(2, 2);
