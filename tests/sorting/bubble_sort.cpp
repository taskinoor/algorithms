...
TEST_F(Sorting, BubbleSortEmptyArray) {
    ASSERT_NO_THROW(alg::sorting::bubble_sort(empty.data(), empty.size()));
}

TEST_F(Sorting, BubbleSortSingleElement) {
    alg::sorting::bubble_sort(single_element.data(), single_element.size());

    ASSERT_EQ(expected_single_element, single_element);
}

TEST_F(Sorting, BubbleSortAlreadySorted) {
    alg::sorting::bubble_sort(already_sorted.data(), already_sorted.size());

    ASSERT_EQ(expected, already_sorted);
}

TEST_F(Sorting, BubbleSortReverseSorted) {
    alg::sorting::bubble_sort(reverse_sorted.data(), reverse_sorted.size());

    ASSERT_EQ(expected, reverse_sorted);
}

TEST_F(Sorting, BubbleSortRandomOrdered) {
    alg::sorting::bubble_sort(random_ordered.data(), random_ordered.size());
